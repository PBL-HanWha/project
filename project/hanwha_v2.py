...
def movex(angle):
    global offset_flag, now_angle_x , offset_x
    if offset_flag == 1 :
        now_angle_x = motor.movex(1, angle,offset_x)
    else :
        now_angle_x = motor.movex(1, angle,0)
    

def movey(angle):
    global offset_flag, now_angle_y, offset_y
    if offset_flag == 1 :
        now_angle_y = motor.movey(2, angle,offset_y)
    else :
        now_angle_y = motor.movey(2, angle,0)
...
def runn(
    ...
    global motor_angle_x, motor_angle_y, mode, angle_x, angle_y, line,  now_angle_x, now_angle_y,count,offset_x,offset_y,turnback_flag,offset_flag, joystick_flag
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det[:1]):
                    ...
                    if now_angle_x < 0   : now_angle_x = 0
                    if now_angle_y > 35 : now_angle_y = 35
                    if now_angle_y < -10 : now_angle_y = -10
                    
                    
                    if (joystick_flag==1):
                        temp = offset_flag
                        offset_flag=0
                        movex(now_angle_x - motor_angle_x)
                        movey(now_angle_y + motor_angle_y)
                        offset_flag = temp
                        joystick_flag = 0
                    ...
                    motor_angle_y = motor.normalize_0_to_180_y(center_y)*2 # 이 화소값을 기반으로 angle값으로 변환 
                        ...
                        #movex(1, now_angle_x - motor_angle_x) # 현재 각도에 x화소 값 차이만큼 이동
                        #movey(2, now_angle_y + motor_angle_y) # 현재 각도에 y화소 값 차이만큼 이동 
                    ...
                    #print('x_error: ', abs(center_x-320))
                    
                    #print('y_error: ', abs(center_y-240))
...
def gui():
    global mode, angle_x, angle_y, line,  now_angle_x , now_angle_y, anglex,angley,gamepad,Isstopped,before,devices,turnback_flag
    tk = Tk()
    ...
    label4 = Label(tk, text='Nvidia Jetson Nano 4GB Development Kit (JetPack 4.6)').grid(row=10, column=0, columnspan=3,
                                                                                         sticky=EW)
    ...
    def mode1():
        global mode, motor_angle_x, motor_angle_y, now_angle_x, now_angle_y, offset_x, offset_y, turnback_flag,offset_flag
        mode = 1
    ...
    def turn(): #수동으로 회전할 때 쓰는 함수 
        ...
        print('\n Turning to ( {} , {} )'.format(angle_x,angle_y))

        movex(angle_x) # motor.move가 movex와 movey로 나눠진 이유 -->> x는 0~180도 y는 0~90도로 제한되니까 상한하한 따로 처리하기 위해 분리함
        movey(angle_y)
        time.sleep(2)
        if (turnback_flag == 1 ):
            time.sleep(3)
    ...
    def patrol(): # 전체 훑을 때 쓰는 함수
        global mode, line, now_angle_x, now_angle_y, offset_flag
        ...
        now_angle_x = motor.movex(1, 90,0)
        now_angle_y = motor.movey(2, 0,0)  # 제자리 정렬
        ...
        for device in devices:
                            ...
                            if device.name==' USB Gamepad          ':
                                ...
                                while True:
                                    ...
                                    if event:
                                        if event.value==1 and event.type==evdev.ecodes.EV_KEY and (event.code==295 or event.code==294): #when pressing front,upper button, mode will change for idle mode
                                        ...
                                        if event.type == 3:
                                            if event.value==127:
                                                ...
                                                if event.value==0:
                                                    ...
                                                    #print(angley)
                                                    if mode==3 : movey(angley)
                                        if event.value==1 and event.type==evdev.ecodes.EV_KEY and (event.code==292 or event.code==293):
                                            #when pressing front,under button motor will rearrange
                                            #temp = offset_flag
                                            #offset_flag = 0 
                                            joystick_flag=1
                                            time.sleep(1)
    ...
    button5 = Button(tk, text='Offset mode', bg='black', fg='white', width=20, command=offset_mode).grid(row=8, column=1,
                                                                                                rowspan=2, sticky=S)
    button6 = Button(tk, text='Offset Reset', bg='black', fg='white', width=20, command=offset_reset).grid(row=8, column=2,
                                                                                                rowspan=2, sticky=S)

    button7 = Button(tk, text='Default', bg='black', fg='white', width=20, command=default).grid(row=8, column=0,
                                                                                                rowspan=2, sticky=S)
