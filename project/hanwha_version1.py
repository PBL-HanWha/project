...
def runn(
    ...
    global motor_angle_x, motor_angle_y, mode, angle_x, angle_y, line,  now_angle_x, now_angle_y,count,offset_x,offset_y
...
def gui():
    ...
    def default():
        global now_angle_x, now_angle_y
        now_angle_x = motor.movex(1,0,0)
        now_angle_y = motor.movey(2,0,0)
    ...
    def mode1():
        global mode, motor_angle_x, motor_angle_y, now_angle_x, now_angle_y, offset_x, offset_y
        mode = 1
        ...
        print(now_angle_y + motor_angle_y)
                        #  now_x - motor_x 여기만 뺄셈인 이유는 모터 회전방향 때문임 
                        #  x화소는 좌측에서 우측으로 갈 수록 증가하지만
                        #  모터가 0도일때 오른쪽을 보고 180도일때 왼쪽을 봐서 서로 반대임
    ...
    def turn(): #수동으로 회전할 때 쓰는 함수 
        global mode, angle_x, angle_y, now_angle_x, now_angle_y , offset_x, offset_y , motor_angle_x, motor_angle_y
